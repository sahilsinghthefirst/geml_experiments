    ...
    return EmlTree(
        ...
        metadata={
            ...
            "restricted_rules": [
                ...
                "1 -> 1",
                "exp(a) -> eml(a, 1)",
                "log(a) -> eml(0, eml(eml(0, a), 1))",
                "add/mul -> eml(log(expr), 1) restricted lift",
            ],
        },
...
def eml_alpha(expr: sp.Expr | str | int) -> float:
    """Return alpha = |T_EML| / |T_AST| for a supported expression."""
    return sympy_to_eml_tree(expr).alpha
...
class _EmlTreeBuilder:
    ...
    def convert(self, expr: sp.Expr) -> int:
        ...
        if expr.func == sp.Add:
            self._validate_nary_args(expr, "add")
            return self._restricted_lift(expr, source_operator="add")

        if expr.func == sp.Mul:
            self._validate_nary_args(expr, "mul")
            return self._restricted_lift(expr, source_operator="mul")
